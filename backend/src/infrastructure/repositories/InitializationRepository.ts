...
export class InitializationRepository {
    ...
    public async executeInitialization(
        ...
        try {
            ...
            await client.query('TRUNCATE TABLE matches, group_teams, groups, teams RESTART IDENTITY CASCADE');
            ...
            `;
            await client.query(settingsQuery, [settings.matchDuration, settings.pauseDuration, settings.vorrundeStartTime]);
            ...
            for (const match of matches) {
                ...
                const endTimeDate = new Date(startTimeDate.getTime() + settings.matchDuration * 60000);

                await client.query(
                    `INSERT INTO matches (id, match_number, home_team_id, away_team_id, group_id, start_time, end_time, status)
                     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
                    [match.id, match.match_number, match.home_team_id, match.away_team_id, match.group_id, match.start_time, endTimeDate.toISOString(), match.status]
                );
            }
