...
import { GroupData, TeamData } from '../../domain/factories/TournamentFactories';
import { CombinedStanding, SeedingAssignment } from '../../domain/models/Tournament';
...
export class GroupRepository {
    ...
    public async fetchGroupByName(name: string, phase: string): Promise<GroupData | null> {
    ...
    public async fetchStandingsForGroup(groupId: string): Promise<any[]> {
        const query = 'SELECT * FROM group_teams WHERE group_id = $1 ORDER BY rank ASC';
        const result = await this.db.query(query, [groupId]);
        return result.rows;
    }
    ...
    public async updateStandings(groupId: string, standings: any[]): Promise<void> {
        for (const stat of standings) {
            ...
            `;
            await this.db.query(query, [stat.points, stat.matches_played, stat.goals_scored, stat.goals_conceded, stat.rank, groupId, stat.team_id]);
        }
    }

    // Diese Methode speichert die Zuordnungen für die Zwischenrunde.
    public async updateAssignedGroups(seeding: SeedingAssignment[]): Promise<void> {
        for (const assign of seeding) {
            const groupQuery = 'SELECT id FROM groups WHERE name = $1 AND phase = $2';
            const groupResult = await this.db.query(groupQuery, [assign.assigned_group, 'ZWISCHENRUNDE']);
    ...
    public async assignFinalRoundGroups(gold: CombinedStanding[], silver: CombinedStanding[]): Promise<void> {
        const goldGroupResult = await this.db.query('SELECT id FROM groups WHERE name = $1 AND phase = $2', ['Goldrunde', 'FINALRUNDE']);
        const silverGroupResult = await this.db.query('SELECT id FROM groups WHERE name = $1 AND phase = $2', ['Silberrunde', 'FINALRUNDE']);
        
        if (goldGroupResult.rows.length > 0) {
            const goldId = goldGroupResult.rows[0].id;
            for (const team of gold) {
                await this.db.query('INSERT INTO group_teams (group_id, team_id) VALUES ($1, $2)', [goldId, team.team_id]);
            }
        }
        ...
        if (silverGroupResult.rows.length > 0) {
            ...
            for (const team of silver) {
                await this.db.query('INSERT INTO group_teams (group_id, team_id) VALUES ($1, $2)', [silverId, team.team_id]);
            }
        }
    }
}